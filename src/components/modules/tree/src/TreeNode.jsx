...
class TreeNode extends React.Component {
	...
	expandNodes() {
		const isExpand = this.props.isExpandAll ? true : false;
		if (this.props.data.children) {
	        if (this.state.children && this.state.children.length)	            {
		this.setState({ children: null, isExpand });
	}	        else	            {
		this.setState({ children: this.props.data.children, isExpand });
	}
	    }
	}
	...
	render() {
		...
		<li className={classes}>
			<a title={this._geTtitle()} onClick={this.onCategorySelect} className='item__description'>
				<i className={iconClasses} onClick={this.onChildDisplayToggle} />
